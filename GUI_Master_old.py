...
#
lbl = tk.Label(root, text="Land Classification On Satellite Images", font=('times', 35,' bold '), height=1, width=60,bg="brown",fg="white")
lbl.place(x=0, y=10)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=350, bd=5, font=('times', 14, ' bold '),bg="#152238")
...
#    global fn
    ...
    if fn!="":
        ...
        if plant == 0:
            Cd="Beach Land Detected"
        elif plant == 1:
            Cd="Forest Land Detected"
        elif plant == 2:
            Cd="Agricultural Land Detected"
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='F:/Softech Solution Code/Land Classification Code/Land Classification100% code/test_set', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
